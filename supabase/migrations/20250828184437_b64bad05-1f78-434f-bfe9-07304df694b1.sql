...
DO $$ 
BEGIN
    -- Agregar shift_type como texto si no existe
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'cash_reconciliation' AND column_name = 'shift_type') THEN
        ALTER TABLE public.cash_reconciliation ADD COLUMN shift_type TEXT NOT NULL DEFAULT 'full_day';
    END IF;
...
-- Crear tipo enum para shift_type si no existe
DO $$ 
BEGIN
...
-- Agregar constraint para evitar duplicados por fecha y turno si no existe
DO $$
BEGIN
    IF NOT EXISTS (
