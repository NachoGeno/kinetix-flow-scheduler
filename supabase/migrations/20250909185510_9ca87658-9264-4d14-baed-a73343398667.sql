...
BEGIN
    ...
    LOOP
        ...
        LOOP
            ...
            -- Calculate how many sessions should have been used by previous orders
            SELECT COALESCE(SUM(mo2.total_sessions), 0)
            INTO sessions_from_previous_orders
            ...
            FOR session_rec IN 
                SELECT a.id, a.appointment_date, a.status,
                       ROW_NUMBER() OVER (ORDER BY a.appointment_date) as session_number
                FROM appointments a
            ...
            LOOP
                -- Adjust session number based on overall patient session count
                -- Sessions before this order's date don't count toward this order
                IF (sessions_from_previous_orders + session_rec.session_number) > sessions_from_previous_orders 
                   AND sessions_for_this_order < order_rec.total_sessions THEN
