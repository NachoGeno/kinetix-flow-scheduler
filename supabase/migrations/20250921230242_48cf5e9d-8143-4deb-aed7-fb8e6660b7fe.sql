-- Fix duplicate key error in appointment_order_assignments
-- Update create_appointments_with_order to use session configuration and handle conflicts
    ...
    LOOP
        BEGIN
            ...
            ELSE
                ...
                -- First delete any existing assignment for this appointment
...
-- Create or update trigger to respect session configuration
CREATE OR REPLACE FUNCTION public.handle_new_appointment_auto_assignment()
RETURNS TRIGGER AS $$
DECLARE
    skip_auto_assign text;
    available_order_id uuid;
BEGIN
    -- Check if auto-assignment should be skipped
    skip_auto_assign := current_setting('app.skip_auto_assign', true);
    
    IF skip_auto_assign = 'true' THEN
        -- Skip auto-assignment when explicitly disabled
        RETURN NEW;
    END IF;

    -- Only auto-assign if no explicit assignment exists
    IF NOT EXISTS (
        SELECT 1 FROM appointment_order_assignments 
        WHERE appointment_id = NEW.id
    ) THEN
        -- Find the most suitable medical order for auto-assignment
        SELECT id INTO available_order_id
        FROM get_medical_orders_with_availability(NEW.patient_id)
        WHERE sessions_remaining > 0
        ORDER BY urgent DESC, created_at ASC
        LIMIT 1;

        -- If we found an order, create the assignment
        IF available_order_id IS NOT NULL THEN
            INSERT INTO appointment_order_assignments (
                appointment_id,
                medical_order_id,
                assigned_by
            ...
            -- Recalculate sessions for the auto-assigned order
            PERFORM recalc_order_sessions(available_order_id);
        END IF;
    END IF;
...
DROP TRIGGER IF EXISTS trigger_auto_assign_appointment_to_order ON appointments;
CREATE TRIGGER trigger_auto_assign_appointment_to_order
