-- FASE 1 - PARTE 2: Crear tabla user_roles, funciones y políticas
-- =================================================================

-- 1. Crear tabla user_roles (segura y separada)
...
-- 2. Migrar datos actuales de profiles.role → user_roles
...
-- 3. Crear función segura has_role() (SECURITY DEFINER)
...
-- 4. Crear función get_user_role_from_table()
...
-- 5. Actualizar función is_admin() para incluir gerencia y leer de user_roles
...
-- 6. Actualizar función get_user_role() para leer de user_roles con fallback
...
-- Super admins y gerencia pueden ver todos los roles
...
-- Solo super_admins pueden insertar/eliminar roles
...
-- Super admins pueden actualizar cualquier rol
CREATE POLICY "Super admins can update all roles"
ON public.user_roles FOR UPDATE
USING (public.has_role(auth.uid(), 'super_admin'));

-- Gerencia puede actualizar roles dentro de su organización (excepto super_admin)
