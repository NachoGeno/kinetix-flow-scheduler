-- Comprehensive Session Counting Fix with FIFO Allocation

-- 1. Audit function to identify discrepancies (read-only)
...
-- 2. FIFO Session Allocation Engine
...
-- 4. Replace the existing trigger with improved logic
DROP TRIGGER IF EXISTS auto_generate_final_summary_trigger ON appointments;
...
CREATE TRIGGER handle_appointment_completion_trigger
    AFTER UPDATE ON appointments
    FOR EACH ROW
    EXECUTE FUNCTION public.handle_appointment_completion();

-- Add helpful comments
COMMENT ON FUNCTION public.audit_patient_session_allocation IS 'Audits session allocation discrepancies using FIFO logic without making changes';
COMMENT ON FUNCTION public.recalc_patient_order_sessions IS 'Recalculates and fixes session counts for a patient using FIFO allocation';
COMMENT ON FUNCTION public.fix_all_patient_session_counts IS 'Fixes session counts for all patients - use for global backfill';
COMMENT ON FUNCTION public.handle_appointment_completion IS 'Handles appointment completion with proper FIFO session allocation and final summary generation';